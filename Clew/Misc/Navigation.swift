...
class Navigation {
    ...
    var lastTimeStampOfDebugInfo = Date()
    ...
    public func getDirections(currentLocation: CurrentCoordinateInfo, nextKeypoint: KeypointInfo, isLastKeypoint: Bool) -> DirectionInfo? {
        ...
        if -lastTimeStampOfDebugInfo.timeIntervalSinceNow > 1.0 {
            shouldDebug = true
            lastTimeStampOfDebugInfo = Date()
        }
        guard let nextLocation = ARSessionManager.shared.getCurrentLocation(of: nextKeypoint.location, debug: shouldDebug) else {
            return nil
        }
