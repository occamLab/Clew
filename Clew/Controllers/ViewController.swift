...
class ViewController: UIViewController, ARSCNViewDelegate, SRCountdownTimerDelegate, AVSpeechSynthesizerDelegate {
    ...
    @objc func updateArrow() {
        ...
        arrow_bez.create2DArrow(omega: CGFloat(directionToNextKeypoint.angleDiff))
        arrowShapeLayer = stopNavigationController.view.layer.sublayers![0] as? CAShapeLayer
        arrowShapeLayer?.path = arrow_bez.cgPath
        
        /// Create 2D arrow
        arrow_bez.createFloatArrow(omega: CGFloat(directionToNextKeypoint.angleDiff))
        arrowShapeLayer = stopNavigationController.view.layer.sublayers![0] as? CAShapeLayer
        arrowShapeLayer?.path = arrow_bez.cgPath
