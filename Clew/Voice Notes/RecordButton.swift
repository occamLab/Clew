...
class RecordButton: UIButton {
    ...
    fileprivate func setAccessibilityLabel() {
        if isRecording {
            self.accessibilityLabel = NSLocalizedString("stopRecordingVoiceNoteAccessabilityLabel", comment: "This is the accessability label which is played when a user with voice over selects the button to stop their voice recording")
        } else {
            self.accessibilityLabel = NSLocalizedString("startRecordingVoiceNoteAccessabilityLabel", comment: "This is the accessability label which is played when a user with voice over selects the button to record a voice recording in the voice recorder view")
        }
    }
